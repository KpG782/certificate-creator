...
// ✅ Generate JWT token (MVP - should be server-side in production)
async function generateJWT(eventId, eventName, expiresAt) {
  const payload = {
    event_id: eventId,
    ...
    <div class="border border-gray-200 rounded-lg p-4 mb-3 hover:shadow-lg transition-shadow bg-gradient-to-r from-white to-gray-50">
      <div class="flex justify-between items-start">
        ...
        <div class="flex gap-2">
          ...
          <button 
            onclick="confirmDeleteEvent('${event.id}', '${event.name.replace(
        /'/g,
        "\\'"
      )}')"
...
window.confirmDeleteEvent = function (eventId, eventName) {
  const confirmed = confirm(
    `⚠️ Are you sure you want to delete the event "${eventName}"?\n\nThis action cannot be undone.`
  );
  if (confirmed) {
    deleteEvent(eventId);
    renderEventsList();
...
window.viewEventQR = async function (eventId) {
  ...
  const displayRegWindowEl = document.getElementById(
    "displayRegistrationWindow"
  );
  if (displayRegWindowEl) {
    displayRegWindowEl.textContent = `${new Date(
      event.registrationStart
    ).toLocaleString()} - ${new Date(event.registrationEnd).toLocaleString()}`;
  }
...
export function initEventManagement() {
  ...
  if (eventForm) {
    eventForm.addEventListener("submit", async (e) => {
      ...
      try {
        submitBtn.innerHTML =
          '<i data-lucide="loader" class="w-5 h-5 mr-2 animate-spin"></i>Generating...';
        ...
        const eventData = {
          ...
          invitedSections: (
            document.getElementById("invitedSections")?.value || ""
          )
            .split(",")
            .map((s) => s.trim())
            .filter((s) => s),
        ...
        // Generate JWT token
        const expiresAt = new Date(eventData.registrationEnd);
        ...
        const attendanceUrl = `${baseUrl}/attend?token=${token}`;

        // Save event with token
        saveEvent({ ...eventData, token, attendanceUrl });
        ...
        await generateQRCode(canvas, attendanceUrl);
        ...
        const displayDateEl = document.getElementById("displayEventDate");
        if (displayDateEl) {
          displayDateEl.textContent = `${new Date(
            eventData.date
        ...
        if (displayLocationEl)
          displayLocationEl.textContent = eventData.location;
        ...
        const jwtTokenEl = document.getElementById("jwtToken");
        if (jwtTokenEl) jwtTokenEl.textContent = token;
